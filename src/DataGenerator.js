...
export default class DataGenerator {
  ...
  __parent(node) {
    ...
    for (let parent in EN[time]) {
      for (let child in EN[time][parent])
        if (EN[time][parent][child] == node) return parent;
    }
    return undefined;
  }
  ...
  __next(node) {
    let { N, ET } = this._data;
    if (!!ET[N[node].streamId]) return ET[N[node].streamId][node];
  ...
  __previous(node) {
    ...
    for (let streamNode in stream)
      for (let next of stream[streamNode])
        if (next == node) prev.push(streamNode);
  ...
  __nodeOrFollowingNodesHaveChildren(node) {
    ...
    if (!!next) return this.__nodeOrFollowingNodesHaveChildren(next);
  ...
  __deleteNode(node) {
    ...
    while ((parent = this.__parent(node))) {
      // we do this because of a bug, normally there should only be one parent
      numParents++;
      if (numParents > 1)
        console.log(
          'WARNING: a node got assigned more than one parent --> this should never have happened'
        );
    ...
    if (!!ET[stream]) {
      for (let streamNode in ET[stream]) {
        ...
        for (let pos = 0; pos < next.length; pos++) {
          if (next[pos] == node) {
            next.splice(pos, 1);
            if (next.length == 0) delete ET[stream][streamNode];
            break;
          }
        }
      }

      if (!!ET[stream][node]) delete ET[stream][node];
  ...
  _genMoveAcross() {
    ...
    let nodes = Object.entries(N).filter(
      ([id, node]) => node.t > 0 && node.modified == false
    );
    ...
    let moveFollowingNodes = (node, time, parent, newParent, movePos = -1) => {
      ...
      if (this.__subtree(node).includes(newParent)) return;
      ...
      if (!!next)
        moveFollowingNodes(
          next[0],
          time + 1,
          this.__next(parent)[0],
          this.__next(newParent)[0],
          movePos
        );
    };
    ...
    for (let i = 0; i < numMoveAcross; i++) {
      ...
      if (typeof nodeToMove == 'undefined') {
        console.log(
          'Delete: Not enough streams to move across. (Moved ' + numMoved + ')'
        );
        break;
      }
      ...
      if (typeof parent == 'undefined') {
        ...
        console.log(
          'ERROR Move across: node has no parent. this state should never be reached'
        );
        continue;
      }
      // the node should not be modified, to not create infinite loops
      let possibleNewParents = Object.entries(EN[time]).filter(
        ([key, val]) =>
          !subtree.includes(key) && key != parent && !N[key].modified
      );
      ...
      if (possibleNewParents.length > 0) {
        ...
        numMoved++;
      } else console.log('Move across: not enough possible parents to move to');
    }
  }
  ...
  _genMoveAlong() {
        ...
        for (let node in EN[t]) {
          ...
          if (children.length > 1) {
            for (let child of children) {
              ...
              if (!N[child].modified) possibleMoves.push(child);
            }
          }
        }
      }
    ...
    let moveFollowingNodes = (nodeToMove, time, parent, movePos = -1) => {
      ...
      if (N[nodeToMove].modified) return;
  ...
  _genAdds() {
    ...
    let removeNodeAndFollowing = node => {
      ...
      let next = this.__next(node);
      if (!!next) removeNodeAndFollowing(next);
    };
    ...
    for (let node in N) {
      if (!!possibleNodes[node]) {
        ...
        if (N[node].modified || N[node].t == timesteps - 1)
          delete possibleNodes[node];
        else if (this.__children(node).length > 0) removeNodeAndFollowing(node);
      }
    }
    ...
    let deleteNodeAndPreviousNodes = node => {
      ...
      if (!!prev) for (let n of prev) deleteNodeAndPreviousNodes(n);
    };
    ...
    for (let i = 0; i < numAdds; i++) {
      ...
      if (possibleNodes.length > 0) {
        ...
        let randomNodeOfParentStream =
          possibleNodes[Math.round(Math.random() * (possibleNodes.length - 1))];
        // add this random node of the stream to our random array at a random position
        remainingNodes.splice(
          Math.round(Math.random() * remainingNodes),
          0,
          randomNodeOfParentStream
        );
      }
    }
  }
  ...
  _genDeletes() {
    ...
    for (let node in N)
      if (
        !N[node].modified &&
        N[node].t > 0 &&
        !this.__nodeOrFollowingNodesHaveChildren(node)
      )
    ...
    let deleteNodeAndFollowingNodes = node => {
      let next = this.__next(node);
      this.__deleteNode(node);
      if (!!next) for (let n of next) deleteNodeAndFollowingNodes(n);
    };
    ...
    for (let i = 0; i < numDeletes; i++) {
      ...
      if (typeof node == 'undefined') {
        console.log(
          'Delete: Not enough streams to delete. (Deleted ' + numDeleted + ')'
        );
        break;
      }
      let parent = this.__parent(node);
  ...
  _genMerges() {
    ...
    let merge = (node, mergeNode) => {
      ...
      if (!!next)
        for (let nextNode of next) merge(nextNode, this.__next(mergeNode)[0]);
    };
    ...
    for (let i = 0; i < numMerges; i++) {
      ...
      if (typeof node == 'undefined') {
        console.log(
          'Merge: Not enough streams to merge. (Merged ' + numMerged + ')'
        );
        break;
      }
      ...
      for (let mergeNode in EN[time])
        if (!subtree.includes(mergeNode)) possibleMerges.push(mergeNode);
    ...
    while (Object.keys(nodesWithoutSize).length > 0) {
      for (let node in nodesWithoutSize) {
        ...
        if (children.length == 0) {
          ...
          let aggregate = 0;
          for (let child of children) {
            if (typeof N[child].w === 'undefined') allChildrenHaveSize = false;
            else aggregate += N[child].w;
          }
          if (allChildrenHaveSize) {
            N[node].w =
              aggregate + minValue + Math.random() * (maxValue - minValue);
            delete nodesWithoutSize[node];
          }
        }
      }
