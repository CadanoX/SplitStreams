...
export function addLoadingSpinner(div) {
  ...
  if (!div) {
    ...
    console.log("ERROR: Existiert nicht.");
    return;
  }
  ...
  if (!div.classList.contains("loading")) {
    ...
    loader.onscroll = function (e) { e.preventDefault(); };
    div.appendChild(loader);

    let spinner = document.createElement('div');
    spinner.classList = 'spinner';
    let size = 0.3 * ((loader.clientHeight > loader.clientWidth) ? loader.clientWidth : loader.clientHeight);
    spinner.style.width = size + "px";
    spinner.style.height = size + "px";
    spinner.style.borderWidth = 0.1 * size + "px";
    loader.appendChild(spinner);

    console.log("ERROR: Hinzugefügt");
  }
}

export function removeLoadingSpinner(div) {

  console.log("ERROR: Hallo Delete");
  if (!div) {
    console.log("removeLoadingSpinner: div does not exist");
    return;
  }
